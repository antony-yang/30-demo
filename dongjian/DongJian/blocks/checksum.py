...
class Checksum(primitives.BasePrimitive):
    ...
    #  "crc16": 2,
    checksum_lengths = {"crc32": 4, "adler32": 4, "md5": 16, "sha1": 20, "ipv4": 2, "udp": 2, "ipv6": 2, "crc-dnp": 2}
    ...
    def __init__(
        ...
        self._endian = endian
        self._name = name
        self._ipv6_block_name = ipv6_block_name
    ...
    def _checksum(self, data, ipv4_src, ipv4_dst):
        ...
        if isinstance(self._algorithm, six.string_types):
            ...
            elif self._algorithm == "ipv6":
                ipv6_data = self._render_block(self._ipv6_block_name)
                ipv6_pseudo_header = ipv6_data[8:] + b"\x00" * 2 + ipv6_data[4:6] + b"\x00" * 3 + ipv6_data[6:7]
                data = ipv6_pseudo_header + data
                check = struct.pack(self._endian + "H", helpers.ipv6_checksum(data))

            elif self._algorithm == "crc-dnp":
                check = struct.pack(self._endian + "H", helpers.crc_for_dnp(data))
    ...
    def original_value(self, value: bytes):
        pass
