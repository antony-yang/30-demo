...
class Session(pgraph.Graph):
    ...
    def __init__(
        ...
        if fuzz_data_logger is not None:
            raise exception.DongJianError("Session fuzz_data_logger is deprecated. Use fuzz_loggers instead!")
        ...
        if os.path.exists(path):
            ...
            os.mkdir(path)
        fuzz_loggers.append(fuzz_logger_text.FuzzLoggerText(file_handle=open(path + self._txt_filename, "w+")))
        ...
        ## pcap mongo
        ###########
        self._current_case_packets = []
        mclient = self._db_handler.getMC()
        mdb = mclient["dongjianpcap"]
        if self.script_start:
            self._buket = GridFSBucket(mdb, bucket_name="script-{0}".format(self._run_id))
        else:
            self._buket = GridFSBucket(mdb, bucket_name="{0}".format(self._run_id))
        ###########
    ...
    def fuzz(self):
        ...
        # #upload
        # report ={'name':self._taskname,'total cases':'','tested cases':''}
        #
        # guan_http.GuanHTTP().post( '', json = report)
    ...
    def fuzz_single_node_by_path(self, node_names):
        ...
        #     if (self.nodes[edge.dst]).num_mutations() > self.total_num_mutations:
        #         self.total_num_mutations = (self.nodes[edge.dst]).num_mutations()
    ...
    def _main_fuzz_loop(self, fuzz_case_iterator):
        ...
        PCAP_PATH = "../pcap"
        if self.script_start:
            if not os.path.exists(PCAP_PATH + "/script-" + self._run_id):
                os.makedirs(PCAP_PATH + "/script-" + self._run_id)
        else:
            if not os.path.exists(PCAP_PATH + "/" + self._run_id):
                os.makedirs(PCAP_PATH + "/" + self._run_id)
        ########
        ...
        try:
            ...
            for fuzz_args in fuzz_case_iterator:
                ...
                ####
                if self.script_start:
                    pcapfile = "{0}/script-{1}/script-{1}-{2}.pcap".format(PCAP_PATH, self._run_id, self.num_fuzzed)
                else:
                    pcapfile = "{0}/{1}/{1}-{2}.pcap".format(PCAP_PATH, self._run_id, self.num_fuzzed)
                wrpcap(pcapfile, self._current_case_packets)
                self.uploadpcap(pcapfile)
                os.remove(pcapfile)
                ###
    ...
    def _restart_target(self, target):
        ...
        @raise exception.DongJianRestartFailedError if restart fails.
        """
    ...
    def transmit_normal(self, sock, node, edge, callback_data):
        ...
        try:  # send
            ...
            self.last_send = data
            ### add packet
            self.add_current_case_packet(self.targets[0], data)
    ...
    def add_current_case_packet(self, target, data):
        connection = target._target_connection
        if connection.proto == "tcp" or connection.proto == "ssl":
            dst = connection.host
            dport = connection.port

            packet_data_size = 14480  # size is refer to real socket data size
            data_length = len(data)
            if data_length <= packet_data_size:
                packet = Ether() / IP(dst=dst) / TCP(dport=dport) / data
                self._current_case_packets.append(packet)
            else:
                packets = []
                seq = 1
                num = 0
                while data_length > packet_data_size:
                    seq = seq + num * packet_data_size
                    packet = Ether() / IP(dst=dst) / TCP(dport=dport, flags="PA", seq=seq, ack=1, window=64240) / data[
                                                                                                                  num * packet_data_size:(
                                                                                                                                                 num + 1) * packet_data_size]
                    packets.append(packet)
                    data_length = data_length - packet_data_size
                    num = num + 1
                packet = Ether() / IP(dst=dst) / TCP(dport=dport, flags="FPA", seq=seq + packet_data_size, ack=1,
                                                     window=64240) / data[num * packet_data_size:]
                packets.append(packet)
                self._current_case_packets.append(packets)
        elif connection.proto == "udp":
            dst = connection.host
            dport = connection.port
            packet = Ether() / IP(dst=dst) / UDP(dport=dport) / data
            self._current_case_packets.append(packet)
        elif connection.proto == "raw-l2":
            packet = data
            self._current_case_packets.append(packet)
        else:  # TODO test raw-l3
            dst = ''.join(['%02x:' % b for b in connection.l2_dst])[:-1]
            packet = Ether(dst=dst, type=connection.ethernet_proto) / data
            self._current_case_packets.append(packet)

    def uploadpcap(self, pcapfile):
        filename = pcapfile.split("/")[-1]
        with open(pcapfile, "rb") as f:
            buffer = f.read()
            self._buket.upload_from_stream(filename, buffer, chunk_size_bytes=1024 * 1024,
