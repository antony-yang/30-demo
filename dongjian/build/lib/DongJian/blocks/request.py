...
class Request(IFuzzable):
    ...
    def original_value(self, value):
        for i in  range(0, len(self.stack)):
            if isinstance(self.stack[i], Block):
                self.stack[i].original_value = value.stack[i]
            else:
                true_value = value.stack[i].render()
                self.stack[i].original_value = true_value
    ...
    def new_mutate(self):
        mutated = False
        # if self.mutant.fuzzable and self.mutant.new_mutate():
        #     mutated = True
        for i in range(0, len(self.stack)):
            item = self.stack[i]
            if item.fuzzable and item.new_mutate():
                mutated = True
        return mutated
