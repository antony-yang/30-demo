...
class Mirror(BasePrimitive):
    ...
    def original_value(self, value: bytes):
        if self.primitive_name is not None:
            self._request.names[self.primitive_name].original_value = value
        else:
            self._request.names[self.primitive_name].original_value = None
