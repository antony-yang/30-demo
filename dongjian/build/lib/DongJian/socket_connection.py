        ...
        # Connect is needed only for TCP protocols
            ...
            try:
                ...
                if e.errno in [errno.ECONNREFUSED, errno.EINPROGRESS, errno.ETIMEDOUT]:
                    raise exception.DongJianTargetConnectionFailedError(str(e))
                else:
                    raise

        # if SSL is requested, then enable it.
        if self.proto == "ssl":
            # If DongJian is the SSL client and user did not give us a SSLContext,
    ...
    def recv(self, max_bytes):
        ...
        try:
            ...
            if e.errno == errno.ECONNABORTED:
                raise_(
                    exception.DongJianTargetConnectionAborted(socket_errno=e.errno, socket_errmsg=e.strerror),
                    None,
                    sys.exc_info()[2],
                )
            elif (e.errno == errno.ECONNRESET) or (e.errno == errno.ENETRESET) or (e.errno == errno.ETIMEDOUT):
                raise_(exception.DongJianTargetConnectionReset(), None, sys.exc_info()[2])
            ...
            if self.proto == "tcp":
                ...
                if self.server:
                    if self._udp_client_port is None:
                        raise exception.DongJianError("recv must be called before send with udp fuzzing servers.")
