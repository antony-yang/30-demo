...
class CrashBinning:
    ...
    def record_crash(self, pykd, extra=None):
        """
        Given a PyDbg instantiation that at the current time is assumed to have "crashed" (access violation for example)
        record various details such as the disassemly around the violating address, the ID of the offending thread, the
        call stack and the SEH unwind. Store the recorded data in an internal dictionary, binning them by the exception
        ...
        # add module name to the exception address.
        register = pykd.dbgCommand("r")
        register = register.split('\n')
        exception_module = register[-3]

        # try:
        #     disasm_around = pykd.dbgCommand("u")
        #     disasm_around = disasm_around.split('\n')
        # except:
        #     disasm_around = []

        # seh_unwind = pykd.dbgCommand("dt")
        # print seh_unwind
        # print pykd.dbgCommand("u")
        # seh_unwind = seh_unwind.split('\n')

        if exception_module:
            exception_module = exception_module.split('\\')[-1].encode('utf-8')
        else:
            exception_module = "[INVALID]"
        crash.exception_module = exception_module

        crash.exception_address = pykd.getLastException().exceptionAddress

        # crash.exception_code = hex(pykd.getLastException().exceptionCode)[:-1]
        crash.exception_code = hex(pykd.getLastException().exceptionCode)

        '''if len(pykd.getLastException().parameters) > 1:
            crash.write_violation = pykd.getLastException().parameters[0]
            crash.violation_address = pykd.getLastException().parameters[1]
        else:
            crash.write_violation = pykd.getLastException().parameters[0]
            crash.violation_address = 0'''

        crash.write_violation = pykd.getLastException().parameters[0]
        if crash.write_violation == 0:
            crash.violation_address = 0
        else:
            crash.violation_address = pykd.getLastException().parameters[1]

        crash.violation_thread_id = pykd.getThreadSystemID()

        crash.context = {}

        crash.context_dump = register[:-1]

        cmd = "u %s l1" % hex(crash.exception_address)  # [:-1]
        try:
            disasm = pykd.dbgCommand(cmd).split('\n')[-2]
        except:
            disasm = ''
        crash.disasm = disasm

        try:
            # print crash.exception_address
            cmd = "ub %s l10" % hex(crash.exception_address)  # [:-1]
            # print cmd
            disasm_around = pykd.dbgCommand(cmd).split('\n')[1:-1]
            # print disasm_around
            cmd = "u %s l10" % hex(crash.exception_address)  # [:-1]
            disasm_around = disasm_around + pykd.dbgCommand(cmd).split('\n')[1:-1]
        except:
            disasm_around = []
        crash.disasm_around = disasm_around

        # crash.disasm_around = []

        # pykd.setSymbolPath("https://msdl.microsoft.com/download/symbols")

        stack = pykd.dbgCommand("kv")
        stack = stack.split('\n')
        crash.stack_unwind = stack[1:-1]

        ret = pykd.dbgCommand(".exr eip")
        # print ret
        crash.seh_unwind = [ret]
    ...
    def export_file(self, file_name):
        ...
        json.dump(self.bins, open(file_name, "w+"), default=self.handle_class)  # default=lambda o: o.__dict__

        #############################    joshwoo added
        try:
            self._m_db_collection = self._m_db[file_name + "-crash_bin"]
            if len(self.bins) > 0:
                json_bin = json.loads(json.dumps(self.bins, default=self.handle_class))
                self._m_db_collection.insert(json_bin)
