...
class Wsession(object):
    ...
    def insert_log(self, task_id, operation, information):
        log = self._m_db["log"]
        log_info = {}
        log_info["task_id"] = task_id
        log_info["operation"] = operation
        log_info["information"] = information
        log_info["timestamp"] = datetime.datetime.now()
    ...
    def create_and_start_task(self, p):
        ...
        if len(self.processes) >= 5:
            self.insert_log(task_id="", operation="create and start task",
        ...
        if self.start_task({"run_id": run_id})["res"] == 1:
            self.insert_log(task_id=run_id, operation="create and start task", information="success")
            return {"code": 200, "msg": "success", "token": web_config.token, "task_id": run_id}
        else:
            self.insert_log(task_id=run_id, operation="create and start task",
    ...
    def control(self, p):
        try:
            ...
            self.print_trace_back()
            self.insert_log(task_id="", operation="control task",
        ...
        if sig == "01":
            for taskid in p['taskIds']:
                if self.suspend_task_by_name({"run_id": taskid})["res"] == 0:
                    flag = 0
                    self.insert_log(task_id=taskid, operation="suspend task",
                                    information="Failed to suspend task.")
                self.insert_log(task_id=taskid, operation="suspend task", information="success.")
            if flag == 1:
        ...
        if sig == "02":
            for taskid in p['taskIds']:
                if self.resume_task_by_name({"run_id": taskid})["res"] == 0:
                    flag = 0
                    self.insert_log(task_id=taskid, operation="resume task",
                                    information="Failed to resume task.")
                self.insert_log(task_id=taskid, operation="resume task", information="success.")
            if flag == 1:
                return {"code": 200, "msg": "success", "token": web_config.token}
        ...
        if sig == "03":
            for taskid in p['taskIds']:
                if self.kill_task_by_name({"run_id": taskid})["res"] == 0:
                    flag = 0
                    self.insert_log(task_id=taskid, operation="kill task",
                                    information="Failed to kill task.")
                self.insert_log(task_id=taskid, operation="kill task", information="success.")
            if flag == 1:
                return {"code": 200, "msg": "success", "token": web_config.token}
            return {"code": 400, "msg": "Failed to stop some tasks.", "token": web_config.token}
        self.insert_log(task_id="", operation="control task", information="Wrong signal!")
    ...
    def status(self, p):
        try:
            ...
            self.print_trace_back()
            self.insert_log(task_id="", operation="get status", information="Lacking parameter " + str(e))
            return {"code": 400, "msg": "Lacking parameter " + str(e), "token": web_config.token}
        ...
        try:
            ...
            for taskid in p['taskIds']:
                ...
                self.insert_log(task_id=taskid, operation="get status", information="success")
            return {"code": 200, "msg": "success", "token": web_config.token, "status": status,
                    "old_status": old_status}
        except Exception as e:
            self.print_trace_back()
            self.insert_log(task_id="", operation="get status", information=str(e))
    ...
    def progress(self, p):
        try:
            ...
            self.print_trace_back()
            self.insert_log(task_id="", operation="get progress", information="Lacking parameter " + str(e))
            return {"code": 400, "msg": "Lacking parameter " + str(e), "token": web_config.token}
        percent = []
        try:
            task_list_db = self._m_db["task_list"]
            for taskid in p['taskIds']:
                ...
                if task["status"] == "finished":
                    percent.append(100)
                self.insert_log(task_id=taskid, operation="get progress", information="success")
            return {"code": 200, "msg": "success", "token": web_config.token, "percent": percent}
        except Exception as e:
            self.print_trace_back()
            self.insert_log(task_id="", operation="get progress", information=str(e))
    ...
    def push_task_result1(self, run_id):
        ...
        args = {'taskId': run_id, 'token': web_config.token, "type": 1, 'resdata': result}
        url = web_config.result_push_url
        request = GuanHTTP()
        try:
            response = request.post(url=url, json=args)
            if response["res"] == 0:
                self.insert_log(task_id="", operation="push type 1 result", information=str(response["value"]))
                logger.error(str(response["value"]))
            else:
                self.insert_log(task_id="", operation="push type 1 result", information="success")
    ...
    def get_task_result1(self, p):
        try:
            ...
            self.insert_log(task_id="", operation="get task result, type 1", information="Lacking parameter " + str(e))
            return {"code": 400, "msg": "Lacking parameter " + str(e), "token": web_config.token}
        run_id = p['taskId']
        task_list_db = self._m_db["task_list"]
        task = task_list_db.find_one({"run_id": run_id}, {"_id": 0})
        if task["status"] != "finished":
            self.insert_log(task_id=run_id, operation="get task result, type 2",
                            information="The task is not fininshed.")
            return {"code": 400, "msg": "The task is not fininshed.", "token": web_config.token}
        result = self.type1_result(run_id)
        self.insert_log(task_id=run_id, operation="get task result, type 1", information="success")
    ...
    def push_task_result2(self, run_id):
        ...
        args = {'taskId': run_id, 'token': web_config.token, "type": 2}
        url = web_config.result_push_url
        request = GuanHTTP()
        try:
            response = request.post(url=url, json=args, files=file)
            if response["res"] == 0:
                self.insert_log(task_id="", operation="push type 2 result", information=str(response["value"]))
                logger.error(str(response["value"]))
            else:
                self.insert_log(task_id="", operation="push type 2 result", information="success")
    ...
    def get_task_result2(self, p):
        try:
            ...
            self.print_trace_back()
            self.insert_log(task_id="", operation="get task result, type 2", information="Lacking parameter " + str(e))
            return {"code": 400, "msg": "Lacking parameter " + str(e), "token": web_config.token}
        run_id = p['taskId']
        task_list_db = self._m_db["task_list"]
        task = task_list_db.find_one({"run_id": run_id}, {"_id": 0})
        if task["status"] != "finished":
            self.insert_log(task_id=run_id, operation="get task result, type 2", information="The task is not fininshed.")
            return {"code": 400, "msg": "The task is not fininshed.", "token": web_config.token}
        filename = "../zip/" + run_id + ".tar.gz"
        if not os.path.exists(filename):
    ...
    def push_task_result3(self, run_id):
        args = {'taskId': run_id, 'token': web_config.token, "type": 3, "levels": None, "shuls": None, "types": None}
        url = web_config.result_push_url
        request = GuanHTTP()
        try:
            response = request.post(url=url, json=args)
            if response["res"] == 0:
                self.insert_log(task_id="", operation="push type 3 result", information=str(response["value"]))
                logger.error(str(response["value"]))
            else:
                self.insert_log(task_id="", operation="push type 3 result", information="success")
    ...
    def get_task_result3(self, p):
        try:
            ...
            self.print_trace_back()
            self.insert_log(task_id="", operation="get task result, type 3", information="Lacking parameter " + str(e))
            return {"code": 400, "msg": "Lacking parameter " + str(e), "token": web_config.token}
        run_id = p['taskId']
        task_list_db = self._m_db["task_list"]
        task = task_list_db.find_one({"run_id": run_id}, {"_id": 0})
        if task["status"] != "finished":
            self.insert_log(task_id=run_id, operation="get task result, type 3", information="The task is not fininshed.")
            return {"code": 400, "msg": "The task is not fininshed.", "token": web_config.token}
        self.insert_log(task_id=run_id, operation="get task result, type 3", information="success.")
    ...
    def push_status_change(self, run_id, status, old_status):
        ...
        args = {'taskId': run_id, 'token': web_config.token, "statusid": status_dic[status],
                "ostatusid": status_dic[status]}
        url = web_config.status_change_url
        request = GuanHTTP()
        try:
            response = request.post(url=url, json=args)
            if response["res"] == 0:
                self.insert_log(task_id="", operation="push status change", information=str(response["value"]))
                logger.error(str(response["value"]))
            else:
                self.insert_log(task_id="", operation="push status change", information="success")
    ...
    def log(self, p):
        ...
        try:
            ...
            self.print_trace_back()
            self.insert_log(task_id="", operation="get log", information="Lacking parameter " + str(e))
            return {"code": 400, "msg": "Lacking parameter " + str(e), "token": web_config.token}
        start_time = datetime.datetime.strptime(p['times'].split(',')[0], '%Y%m%d')
        end_time = datetime.datetime.strptime(p['times'].split(',')[1], '%Y%m%d')
        log_list = log.find({'timestamp': {'$gt': start_time, '$lt': end_time}}, {"_id": 0})
        log_result = []
        for log_info in log_list:
            log_result.append(log_info)
        self.insert_log(task_id="", operation="get log", information="success")
        return {"code": 200, "msg": "success", "log": log_result, "token": web_config.token}
    ...
    def delete_tasks(self, p):
        try:
            ...
            self.print_trace_back()
            self.insert_log(task_id="", operation="delete task", information="Lacking parameter " + str(e))
            return {"code": 400, "msg": "Lacking parameter " + str(e), "token": web_config.token}
        flag = 1
        for task_id in p['taskIds']:
            if self.delete_task({"run_id": task_id})["res"] == 0:
                flag = 0
                self.insert_log(task_id=task_id, operation="delete task", information="Something wrong")
            self.insert_log(task_id=task_id, operation="delete task", information="success")
        if flag == 1:
            return {"code": 200, "msg": "success", "token": web_config.token}
    ...
    def backup_data(self, p):
        try:
            ...
            self.print_trace_back()
            self.insert_log(task_id="", operation="backup data", information="Lacking parameter " + str(e))
            return {"code": 400, "msg": "Lacking parameter " + str(e), "token": web_config.token}
        time = datetime.datetime.strptime(p['times'], '%Y%m%d').isoformat().replace(":", "-").replace(".", "-")
        ...
        try:
            ...
            if response["res"] == 0:
                self.insert_log(task_id="", operation="backup data", information=str(response["value"]))
                logger.error(str(response["value"]))
            else:
                self.insert_log(task_id="", operation="backup data", information="upload file success")
                logger.info("upload file success")
        except Exception as e:
            print("can't reach result_push_url")
        # shutil.rmtree(BACKUP_PATH)

    def restore_data(self, data_date):
        RESTORE_PATH = "../backup"
        ...
        # decompress
        decompress_cmd = "cd {};/bin/tar xzf {}.tar.gz".format(RESTORE_PATH, data_date)
        ...
        load_mongo_cmd = "cd {};" + web_config.mongodb_location + "mongorestore -h {} -u {} -p {} -d {} --drop --authenticationDatabase admin {}/mongo/{}".format(
            RESTORE_PATH,
            db_config.db_host,
            db_config.db_user,
            db_config.db_passwd,
        ...
        self.insert_log(task_id="", operation="retore data", information="success")
