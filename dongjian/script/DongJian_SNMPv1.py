...
def fuzz(start_cmds, proc_name, target_ip, pport, dport, *args, **kwargs):
    ...
    if s_block_start('snmp header'):
        s_static('\x02\x01')
        s_static(name='version', value='\x00')
        s_static('\x04\x06')
        ...
        if s_block_start('header'):
            ...
            s_static('\x02\x01')
            s_bit(name='error index', value=0, width=1, fuzzable=False)
            s_static('\x30')
    ...
    if s_block_start('snmp trap'):
        ...
        if s_block_start('trap'):
            s_static('\x06')
            s_size(block_name='enterprise', length=1, endian='>', inclusive=False, name='enterprise len',
                   fuzzable=False)
            ...
            if s_block_start('trap var all'):
                s_static('\x30')
                s_size(block_name='trap object name', length=1, endian='>', inclusive=False, name='trap object len',
                       fuzzable=False)
                if s_block_start('trap object name'):
                    s_static('\x06')
                    s_size(block_name='object iso', length=1, endian='>', inclusive=False, name='object iso len',
                           fuzzable=False)
                ...
                s_static('\x30')
                s_size(block_name='trap object1 name', length=1, endian='>', inclusive=False, name='trap object1 len',
                       fuzzable=False)
                if s_block_start('trap object1 name'):
                    s_static('\x06')
                    s_size(block_name='object1 iso', length=1, endian='>', inclusive=False, name='object1 iso len',
                           fuzzable=False)
                    if s_block_start('object1 iso'):
                        s_group(name='trap1 iso', values=['\x2b\x04\x01\x02\x03\x01'])
                    s_block_end('object1 iso')
                    s_static('\x04')
                    s_size(block_name='trap value2', length=1, endian='>', inclusive=False, name='trap value2 len',
                           fuzzable=False)
