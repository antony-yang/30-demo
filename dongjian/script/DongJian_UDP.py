...
def fuzz(start_cmds, proc_name, target_ip, pport, dport, *args, **kwargs):
    ...
    s_initialize("IP_Protocol")
    if s_block_start("udp"):
        if s_block_start("IP_Header"):
            s_static(value='\x45', name='Version')
            s_static(value='\x00', name='TOS')
            s_size(name='Total Length', block_name='udp', length=2, inclusive=False, endian='>', fuzzable=False)
        ...
        if s_block_start('udp_protocol'):
            s_word(value=12345, endian='>', fuzzable=True, name='SrcPort')
            s_word(value=443, endian='>', fuzzable=True, name='DstPort')
            s_size(name='udp length', block_name='udp_protocol', length=2, inclusive=False, fuzzable=False, endian='>')
            s_checksum(block_name='udp_protocol', algorithm='udp', length=2, endian='>',
                       ipv4_src_block_name='src', ipv4_dst_block_name='dst', fuzzable=False)

            s_string(value='11111', name='Data', padding=b"\u0000", max_len=1500)
        s_block_end(name='udp_protocol')
    s_block_end("udp")

    sess.connect(s_get('IP_Protocol'))
    sess.fuzz()
...
if __name__ == "__main__":
    ...
    pport = 0
    dport = 26002
    fuzz(start_cmds, proc_name, target_ip, pport, dport, script_start=True, net_interface="ens33", l2_dst="080907060504")
