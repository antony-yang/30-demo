...
def fuzz(start_cmds, proc_name, target_ip, pport, dport, *args, **kwargs):
    ...
    s_byte(0,signed=True,full_range=True) #groupaddr
    s_byte(0,signed=True,full_range=True) #groupaddr
    s_byte(0,signed=False) #Resv(3) + S(1) + QRV(4)
    s_byte(0,signed=False) #QQIC
    s_word(0)              #Number of Sources (N)
    s_dword(0)             #Source Address [i]
    s_block_end("igmp")
    s_block_end("ipv4")

    session.connect(s_get("IGMPv3"))
    session.fuzz()
